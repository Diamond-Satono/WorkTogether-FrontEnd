<template>
    <div class="root">
        ...
        <div class="modal-content">
            <div class="title">
                新建团队<img src="@/assets/deptimgs/closeModal.png" class="closeModal" @click="$emit('close-modal')" />
            </div>
            <div class="cutoff"></div>
            <div class="maincontent">
                <div class="deptname">
                    团队名称<input type="text" placeholder="请填写团队名称" class="nameinput" v-model="groupName" />
                </div>
                <div class="deptprincipal">团队负责人<select v-model="selectedMember" class="select">
                        <option value="" disabled selected>请选择团队负责人</option> <!-- 添加默认选项 -->
                        <option v-for="member in members" :key="member.id" :value="member.name">{{ member.name }}
                        </option>
                    </select></div>
                <div class="buttoncontainer">
                    <button class="create" @click="createandclose">确定添加</button>
                </div>
            </div>
        </div>
    </div>
...
async function createGroup() {
    ...
    try {
        ...
        // 发送 POST 请求创建团队
        const response = await fetch('http://localhost:8080/api/group/createGroup', {
            method: 'POST',
            headers: {

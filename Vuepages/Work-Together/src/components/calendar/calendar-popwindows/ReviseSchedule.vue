<template>
  ...
  export default {
    ...
    created() {
      const now = new Date();
      this.startYearMonth = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`;
    ...
    methods: {
      loadSchedule() {
        ...
        .then(data => {
          ...
          this.startDateTime = schedule.startTime;
          this.endDateTime = schedule.endTime;
          this.calendarType = schedule.type.toString();
          this.description = schedule.description;
          this.participants = schedule.userIds.map(userId => {
            // 查找并返回成员信息
            return this.members.find(member => member.id === userId);
          }).filter(participant => participant); // 过滤掉未找到的用户
        })
        .catch(error => {
